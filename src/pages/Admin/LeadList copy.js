...
const LeadList = () => {
  ...
  const handleDeleteLead = async (id) => {
    try {
      ...
      if (response.status === 200) {
        ...
        console.error(
          `Failed to delete lead with id ${id}. Status: ${response.status}`
        );
      }
    } catch (error) {
  ...
  const handleAssignLeads = async (assignedTo) => {
    ...
    try {
      const updateLeadIds = Array.isArray(selectedLeads)
        ? selectedLeads
        : [selectedLeads];
      ...
      console.log(assignResponse.data.assignedTo.name, "assignResponse");
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    // Get LeadAssignments for the current lead
    ...
    // Assuming employees is available in the scope
  ...
  const currentRecords = sortedLeads.slice(
    indexOfFirstRecord,
    indexOfLastRecord
  );
